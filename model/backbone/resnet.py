...
def _resnet(arch, block, layers, pretrained, **kwargs):
    ...
    if pretrained:
        import os
        # First try a local checkpoint (cwd or the directory above this file)
        for base in [os.getcwd(), os.path.dirname(os.path.dirname(os.path.abspath(__file__)))]:
            pretrained_path = os.path.join(base, "pretrained", "%s.pth" % arch)
            if os.path.isfile(pretrained_path):
                state_dict = torch.load(pretrained_path, map_location="cpu", weights_only=False)
                model.load_state_dict(state_dict, strict=False)
                break
        else:
            # Fallback: load ImageNet weights from torchvision where layer names and shapes match
            try:
                from torchvision.models import resnet101, ResNet101_Weights
                tv_model = resnet101(weights=ResNet101_Weights.IMAGENET1K_V2)
                tv_sd = tv_model.state_dict()
                our_sd = model.state_dict()
                loaded = {k: v for k, v in tv_sd.items() if k in our_sd and our_sd[k].shape == v.shape}
